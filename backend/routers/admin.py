...
def reply_to_inquiry(
    ...
    db: Session = Depends(get_db),
    current_admin: Admin = Depends(get_current_admin)  # 현재 로그인한 관리자 정보 사용
):
    """
    [관리자 기능] 특정 문의에 답변 등록/수정
    
    Headers:
    - **Authorization**: Bearer {token}
    """
    # 현재 로그인한 관리자의 ID를 사용하여 답변 등록
    admin_id = current_admin.admin_id
    
    inquiry, reply, is_created = inquiry_service.create_or_update_reply(
        db, inquiry_id, admin_id, reply_data
    )
    
    if not inquiry:
        raise HTTPException(
            status_code=404, 
            detail="해당 문의를 찾을 수 없습니다."
        )

    return {
        "success": True,
        "message": "답변이 등록되었습니다." if is_created else "답변이 수정되었습니다.",
        "data": {
