  ...
  const submitFeedback = async (isHelpful) => {
    try {
      ...
      if (res.data.success) {
        ...
        showToast(res.data.message || "피드백이 제출되었습니다.", "success");
      } else {
        showToast("피드백 제출 실패: " + (res.data.message || ""), "error");
      }
    } catch (error) {
      console.error("피드백 제출 오류:", error);

      const msg =
        error.response?.data?.message || "피드백 제출 중 오류가 발생했습니다.";
