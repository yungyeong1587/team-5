...
const POSITIVE_KEYWORDS = [
  "좋아요", "좋았", "좋은", "만족", "추천", "예쁘", "이쁘", "맘에 들어", "맘에들", "잘 맞", "잘맞",
  "편하", "편해", "따뜻", "최고", "괜찮", "만족도", "존예", "존예탬", "존맛", "존좋", "미쳤다",
  "미쳤어요", "미쳤음", "미쳤네", "대박", "대박이야", "레전드", "찐이다", "인생템", "최애템", "최애",
  "갓템", "강추", "강력추천", "재구매", "또 살", "재구매각", "역대급", "혜자", "가성비 좋", "갓성비",
  "퀄리티 좋", "퀄리티 미쳤", "핏 예쁘", "핏이 미쳤", "핏 미쳤", "색감 예쁘", "색감 미쳤", "부드럽",
  "쫀쫀", "포근", "따숩", "따수워", "데일리로 딱", "데일리템", "매일 입게됨", "손이 자주 가요", "느좋",
  "찰떡", "찰떡이에요", "찰떡이네", "찰떡핏", "핏이 진짜", "핏 짱", "핏 최고", "핏 굿", "핏 완전",
  "완전 만족", "완전 좋아", "완전 추천", "진짜 예쁨", "진짜 좋아요", "진짜 편함", "진짜 괜찮",
  "진짜 미쳤", "인생핏", "인생착용", "인생템 인정", "기대 이상", "기대한 것 이상", "예상 이상",
  "생각보다 좋", "생각한 것보다 좋", "색감 대박", "색감 존예", "색감 굿", "색감 예뻐요", "착용감 좋",
  "착용감 미쳤", "촉감 좋", "촉감 부드럽", "질감 좋", "재질 좋", "재질 굿", "재질 미쳤", "탄탄",
  "탄탄하", "가볍고 좋", "가벼워서 좋", "가볍고 편해", "여리여리", "여리핏", "부담없", "무난하게 좋",
  "무난템", "실물이 더 예쁘", "실물이 미쳤", "사진보다 예쁨", "사진이랑 똑같", "사진과 동일",
  "빠른 배송", "배송 빨랐", "배송 굿", "포장 깔끔", "포장 예쁨", "가격 대비 굿", "가격 대비 괜찮",
  "가격대비 최고", "가성비 갑", "마음에 쏙", "마음에 쏙 들어요", "너무 만족", "너무 좋아요",
  "너무 이뻐요", "너무 예뻐요", "너무 좋아서", "너무 마음에", "완전 마음에", "완전 예쁨", "완전 만족",
  "완전 굿", "세련된", "깔끔하", "다양하게 코디 가능", "코디하기 좋", "어디에나 잘 어울", "데일리로 최고",
  "데일리 추천", "데일리로 자주 입", "재구매 의사", "또 사고 싶어요", "또 구매할 예정", "대만족",
  "괜찮음", "괜찮아요",
];

// 👎 부정 키워드들
const NEGATIVE_KEYWORDS = [
  "불만족", "별로", "최악", "실망", "실망스러", "안 좋", "안좋", "아쉽", "불편", "다시 사지",
  "다시는 사지", "비추", "후회", "짜증", "별로였", "별로에요", "최악이었", "별로였어요", "구림",
  "구려요", "구렸", "구리", "구리네요", "엉망", "엉망이었", "형편없", "형편없었", "형편없네요",
  "거지같", "별로임", "별로임니다", "다신 안", "다시는 안", "두번 안", "두 번 다시", "돈 아까",
  "돈아까", "돈값 못", "돈값 못하", "돈값 못함", "돈낭비", "비싸기만", "값어치 없", "퀄리티 떨어",
  "퀄리티 별로", "퀄리티 낮", "저렴해 보", "싸보여", "싸보임", "잡티", "보풀", "보풀이", "늘어남",
  "늘어나요", "찢어", "색 다름", "색상이 다름", "사진이랑 다름", "사진과 달라요", "별로다 진짜",
  "노답", "답없", "ㅈ같", "노잼", "구데기", "별로에요 ㅠ", "실망임", "실망각", "실망했어요", "민폐",
  "개별로", "개실망", "개노답", "헛돈", "현타", "핵별로", "쌉별로", "에바", "개에바", "에바임", "애바",
  "오바임", "오바야", "말도안됨", "어이없", "정떨어", "정떨", "정뚝떨", "극혐", "혐오", "진심 별로",
  "진짜 별로", "좀 별로", "좀 아쉽", "망했", "망함", "망했어요", "배송 늦", "배송 느림", "배송 지연",
  "포장 엉망", "구김", "냄새", "냄새 심함", "냄새나요", "오염", "얼룩", "하자", "하자있음",
  "하자 있습니다", "불량", "불량품", "이염", "박음질 엉망", "박음질 별로", "박음질 안좋", "핏 안예쁨",
  "핏 망", "핏 별로", "핏 이상", "불편함", "답답", "꾸깃",
];
  ...
  return parts.map((part, idx) => {
    ...
    if (isNegative) {
      return (
        <mark
          key={idx}
          className="bg-red-100 text-slate-900 rounded px-0.5"
        >
          {part}
        </mark>
      );
    ...
    if (isPositive) {
      return (
        <mark
          key={idx}
          className="bg-blue-100 text-slate-900 rounded px-0.5"
        >
          {part}
        </mark>
      );
...
export default function Result({ result, onBack, onNewAnalyze }) {
  ...
  const ratingFromBackend =
    typeof avg_rating === "number" && !Number.isNaN(avg_rating)
      ? avg_rating
      : null;
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between px-5 py-2 text-[11px] text-slate-500 bg-slate-50 hover:bg-slate-100 border-t border-slate-100 transition-colors"
                  >
                    <span>
                      {isOpen ? "분석 근거 닫기" : "분석 근거 보기"}
                    </span>
                  ...
                  {/* 분석 근거 영역 (샘플) */}
                  {isOpen && (
                    <div className="px-5 py-3 bg-slate-50 border-t border-slate-100 text-[11px] text-slate-600 leading-relaxed">
                      <p className="mb-1 font-semibold text-slate-700">
                        분석 근거 (샘플 데이터)
                      </p>
                      <p>
                        이 영역에는 백엔드에서 계산한 분석 근거가 들어갑니다.
                        예) 리뷰에 포함된 키워드, 사용자 신뢰도, 작성 패턴 등을
                        이용해 신뢰도 점수(
...
function StatusBadge({ label, colorClass, score }) {
  ...
  if (colorClass) {
    if (colorClass === "status-green")
      ...
      finalColorClass = "bg-slate-100 text-slate-600 border-slate-200";
  } else {
    if (score >= 76) {
      finalColorClass = "bg-green-100 text-green-700 border-green-200";
